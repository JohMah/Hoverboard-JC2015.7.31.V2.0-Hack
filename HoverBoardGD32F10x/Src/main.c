...
//	USART_Steer_COM_init();
  ...
  while(1)
    ...
    // Set output
		...
		//Measure battery voltage by polling for test JMA
//		adc_software_trigger_enable(ADC0, ADC_REGULAR_CHANNEL); //Start ADC conversion
//		while (SET != adc_flag_get(ADC0, ADC_FLAG_EOC)){	//Wait for endo of conversion
//		}
		//batteryVoltage = (float)adc_regular_data_read(ADC0) * ADC_BATTERY_VOLT;	//Read the result of the ADC conversion
